...
export default function FeedbackDisplay({
  ...
  function displaySurveyResults(results: any[]) {
    if (results.length < 1) return <div style={{
      textAlign: "center",
      height: "50%"
    ...
    const average = Object.fromEntries(Object.entries(FeedbackRatingEnum).map(([key]) => [key, Math.round((results.reduce((previous, current) => previous + current[key], 0) / results.length) * 100) / 100]))
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <Pagination count={results.length} page={page} onChange={(event, value) => setPage(value)} />
        <div>
          <h3>Email: {results[page - 1].contact}</h3>
          <h3>Response ID: {results[page - 1]._id}</h3>
        </div>
        <div className={styles.feedbackResponses}>
          {Object.keys(FeedbackRatingEnum).map(key => <div key={key} className={styles.feedbackResponse}>
            <h1>{FeedbackRatingEnum[key as keyof typeof FeedbackRatingEnum]}</h1>
            <Box sx={{ position: "relative", display: "inline-flex" }}>
                <CircularProgress
                  variant="determinate"
                  value={100}
              ...
              <h2>Feedback:</h2>
              <p>{results[page - 1][key.replace("StarRating", "Feedback")] || "None provided"}</p>
          </div>)}
        </div>
      </div>
      </div>
    ...
    <Layout title="Feedback">
      <div className={styles.devFeedbackPage}>
        <Select
          ...
          onChange={(event) => {setDisplayBuildId(event.target.value); setPage(1)}}
        >
          <MenuItem value={buildId}>Current Build</MenuItem>
          {ids.reverse().map((id) => id === buildId ? null : (
            <MenuItem key={id} value={id}>
              Build on {new Date(hexToUtf8(id.split("-").at(-1) as string)).toLocaleString()}
            </MenuItem>
          ))}
        </Select>

        <h3>Build ID:<br /><code style={{
          fontWeight: "100"
        }}>{buildId}</code></h3>
        <h3>Total Responses:<br />{(feedback[displayBuildId] ?? []).length}</h3>
